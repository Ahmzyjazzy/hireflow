...
async def display_state(
    ...
    try:
        ...
        user_profile = session.state.get("user:profile", {})
        if isinstance(user_profile, dict):
            user_name = user_profile.get('name', 'Unknown')
        else:
            # If it's a string or other type, treat it as unknown or handle accordingly
            user_name = "Unknown"
        
        print(f"👤 User: {user_name}")

        # Handle checklist
        checklist = session.state.get("user:checklist", {})
...
def clear_db():
    """Clears the SQLite database by removing the file."""
    if os.path.exists("hireflow.db"):
        os.remove("hireflow.db")
